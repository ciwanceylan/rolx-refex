...
def refex_dense(adj: np.ndarray, param: core.RefexParam):
    ...
    adj_dict, _ = core.dense2adj_dict(adj, return_values=False)
...
def extract_base_features(adj: np.ndarray, use_weights=True):
...
def extract_node_features(adj: np.ndarray, use_weights=True):
    ...
    if use_weights:
        out_sum_weight = adj.sum(axis=0)
        in_sum_weight = adj.sum(axis=1)
...
def extract_egonet_features(adj: np.ndarray, use_weights=True):
    ...
    """
    out_adj_dict, out_weights = core.dense2row_adj_dict(adj.T, return_values=True)
    in_adj_dict, in_weights = core.dense2row_adj_dict(adj, return_values=True)
